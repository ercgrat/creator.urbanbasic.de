            ...
            </ul>
            {
                props.cart.getItems().reduce((total, item) => total + item.quantity, 0) > 0 ?
                    <section className={styles.cartSummary}>
                        <p className={styles.cartSummaryItem}><span className={styles.label}> Zwischensumme</span> {formatPrice(props.cart.getSubtotal())}</p>
                        <p className={styles.cartSummaryItem}><span className={styles.label}> Versand</span> {formatPrice(props.cart.getShipping())}</p>
                        <p className={`${styles.cartSummaryItem} ${styles.cartSummaryTotal}`}><span className={styles.label}> Rechnungsbetrag</span> {formatPrice(props.cart.getTotal())}</p>
                    </section> :
